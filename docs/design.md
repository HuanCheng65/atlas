...
│  atlas-entity       atlas-compact (planned)              │
└────────────────────────┬─────────────────────────────────┘
                         │ reads / writes
                         ▼
┌──────────────────────────────────────────────────────────┐
...
`_index.md` files are derived views, regenerated by `reindex.py`.
...
warrant distillation. `atlas-compact` proposes these; user confirms.
...
[in parallel throughout:]
   ...
   atlas-compact    ← weekly: propose promotions, close stale items
```
...
- **Pre-hoc confirmation for every atlas-log append** — rejected. Friction
  kills the flow. Post-hoc transparency (announce one line after append)
  achieves the same accountability with much less overhead. Close still
  requires confirmation because it is sticky.
...
- **Stale active entry threshold.** Currently using "3 days no update" as
  ...
  against real usage.
- **Topic auto-promotion heuristic.** How does `atlas-compact` detect
  "this cluster of journal entries should be sealed into a topic"?
  Candidate heuristics: shared tags across N consecutive entries within
  K days; number of cross-references to a tag exceeding a threshold.
- **`_index.md` git-tracking.** Currently committed (browsable without
  running reindex). Risk: merge conflicts on parallel work. Tolerable
  until friction shows; fallback is `.gitignore` + always-regenerate.
...
- **`bootstrap-extras.md` consumption.** `atlas-bootstrap` writes overflow
  candidates here. No follow-up workflow exists for the user to revisit
  later. Open whether `atlas-compact` should scan extras during its
  weekly run.
