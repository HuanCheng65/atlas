...
supersedes: []
superseded-by: []
affects: [using-atlas, atlas-log, atlas-orient, grill-me, atlas-entity, atlas-bootstrap]
---
...
- Substantive conversation is where context-switching costs are real — a `D-007` reference forces the user to mentally jump into the framework to understand what was meant. Translation is mandatory there.
...
D-001 (plain text + git as the data layer) makes file paths legitimate first-class artifacts, not "implementation detail" — supports keeping them in announces. D-007 (event-driven skill activation) frames lifecycle events as the natural moments for a brief signal — supports the announce convention.
