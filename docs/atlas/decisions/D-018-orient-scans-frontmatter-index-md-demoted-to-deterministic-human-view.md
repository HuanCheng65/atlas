...
supersedes: []
superseded-by: []
affects: [atlas-orient, atlas-log, atlas-entity]
---
...
- orient.py is reworked to scan `decisions/`, `questions/`, `experiments/`, and `journal/` frontmatter directly; this lands together with the D-017 menu changes so orient is rebuilt once.
...
- D-012's "title is the menu" principle is unchanged; what narrows is who reads the rendered menu file: derived views are for humans, the agent reads the facts they derive from.
