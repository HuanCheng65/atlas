...
| One skill per cognitive domain ([[007-one-skill-per-cognitive-domain-deterministic-ops-live-in-scr]]) | One file per work unit ([[019-one-file-per-work-unit-plan-work-log-close-merged-into-a-jou]]) / one entity per decision |
...
- **No principle, let titles drift**: rejected — current state already has uneven titles ([[002-skills-user-level-data-project-level-via-symlink-install]] "Skills user-level, data project-level via symlink install" reads compressed; [[021-user-facing-transparency-two-tier-framework-vocabulary-rule]] v1 leaned topic-ish). Without a written rule, drift continues and orient becomes less useful over time.
- **Encode the principle per-skill**: rejected — atlas-entity and atlas-log would re-derive the same rule independently, with slow drift between them. Single source of truth is cheaper to maintain (same argument that drove [[021-user-facing-transparency-two-tier-framework-vocabulary-rule]]'s single-source approach).
- **Build a section-extract tool now** (`extract.py [[013-event-driven-skill-activation-not-session-phase-driven]] --section Rationale`): deferred. The capability is approximated by Read offsets + Grep + heading patterns; build the tool when convention proves insufficient, not before. Aligns with [[001-plain-text-git-as-the-data-layer]]'s "plain text, don't over-engineer the data layer".
- **Move toward structured data (YAML body, JSON facets)**: rejected — directly contradicts [[001-plain-text-git-as-the-data-layer]]. Plain markdown stays the data layer; structure comes from section conventions, not new formats.
